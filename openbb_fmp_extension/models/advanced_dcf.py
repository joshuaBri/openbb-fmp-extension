...
class FMPAdvancedDcfData(AdvancedDcfData):
    ...
    ebit: Optional[float] = Field(
        default=None, description="Earnings Before Interest and Taxes, representing operating profit after costs."
    )
    ...
    depreciation: Optional[float] = Field(
        default=None, description="Depreciation and amortization expenses spread over asset lifespan."
    )
    ...
    receivables: Optional[float] = Field(
        default=None, description="Accounts receivable, representing amounts owed from sales."
    )
    ...
    payable: Optional[float] = Field(
        default=None, description="Accounts payable, representing amounts owed for purchased goods or services."
    )
    ...
    tax_rate_cash: Optional[float] = Field(
        default=None, description="Cash tax rate, representing the effective tax rate paid by the company."
    )
    ebiat: Optional[float] = Field(
        default=None, description="Earnings Before Interest After Taxes, representing operating earnings after tax."
    )
    ...
    sum_pv_ufcf: Optional[float] = Field(
        default=None, description="Total present value of future unlevered free cash flows."
    )
    operating_cash_flow: Optional[float] = Field(
        default=None, description="Operating cash flow generated from core business activities."
    )
    pv_lfcf: Optional[float] = Field(
        default=None, description="Present value of levered free cash flow, discounted at appropriate rate."
    )
    ...
    free_cash_flow: Optional[float] = Field(
        default=None, description="Levered Free Cash Flow, representing cash flow after interest and capex."
    )
    operating_cash_flow_percentage: Optional[float] = Field(
        default=None, description="Percentage of operating cash flow relative to revenue."
    )
...
class FMPAdvancedDcfFetcher(
    ...
    async def aextract_data(
        ...
        async def get_one(symbol):
            ...
            if debt:
                url = create_url(
                    4, f"advanced_levered_discounted_cash_flow", query, exclude=["debt"]
                )
            else:
                url = create_url(
                    4, f"advanced_discounted_cash_flow", query, exclude=["debt"]
                )
