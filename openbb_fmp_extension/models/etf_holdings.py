    ...
    async def aextract_data(
        ...
        **kwargs: Any,
        ...
        if query.date is not None:
            url = create_url(
                version=4, endpoint="etf-holdings", query=query
            )
            try:
        ...
        if query.date is None or not data:
            # url = f"https://financialmodelingprep.com/api/v3/etf-holder/{query.symbol}"
            url = create_url(version=3, endpoint=f"etf-holder/{query.symbol}", query=query, exclude=["symbol"])
            data = get_jsonparsed_data(url)
        return data
